...
class ServicePosition(Poll, Converter, object):
	...
	TYPE_REMAINING = 2
	TYPE_GAUGE = 3
#	ikseong fix to show length, position	
	prev_length=0
	prev_position=0
#
...
#		print "pos",pos[0],pos[1]
		if pos[1]!= 0 and pos[0]==-1:
			pos[0] = 0
			pos[1]=self.prev_position
		elif pos[0]==0:
			self.prev_position = pos[1]
#			
		if pos[0]:
			return 0
		return pos[1]
...
#	ikseong fix to show length, position	
#		print "length",length[0],length[1]
		if length[0]==-1:
			length[0] = 0
			length[1]=self.prev_length
#		elif length[1] == 0:
#			length[1] = 0
		elif length[0]==0:
			self.prev_length=length[1]
#			
		if length[0]:
			return 0
		return length[1]
