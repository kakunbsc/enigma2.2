...
class ScanSetup(ConfigListScreen, Screen, CableTransponderSearchSupport):
	...
	def __init__(self, session):
		...
		self.createSetup()
		#	ikseong
		if self.tuner_status==1:
			self["introduction"] = Label(_("Press OK to start the scan"))
		else:
			self["introduction"] = Label(_("No Tuner Information"))
		#
...
class ScanSimple(ConfigListScreen, Screen, CableTransponderSearchSupport):
	...
	def __init__(self, session):
		...
		if len(nims_to_scan):
			...
			#self.scan_networksearch = ConfigSelection(default = "yes", choices = [("no", _("no")), ("yes", _("yes"))])
			#self.list.append(getConfigListEntry(_("Network search"), self.scan_networksearch))
