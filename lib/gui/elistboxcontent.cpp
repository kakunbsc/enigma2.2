...
void eListboxPythonStringContent::paint(gPainter &painter, eWindowStyle &style, const ePoint &offset, int selected)
{
	...
	if (m_list && cursorValid)
	{
		...
		} else
		{
...
#ifdef BUILD_VUPLUS /* ikseong  */
			if(m_spoint!=ePoint(0,0))
				text_offset += m_spoint;
#endif
			if (gray)
			...
			if(m_spoint!=ePoint(0,0))
			{
				eSize tsize(m_spoint.x()*3,0);
				if(m_itemsize.width()>(m_spoint.x()*3))
					painter.renderText(eRect(text_offset, (m_itemsize-tsize)), string);
				else
...
void eListboxPythonStringContent::setFont(gFont *font)
{
	m_font = font;
}

void	eListboxPythonStringContent::setSPosition(ePoint *start)
{
	m_spoint = *start;
}
#endif
...
#ifdef BUILD_VUPLUS /* ikseong  */
			if(local_style->m_selection)
			{
				offset.setX(offset.x()+3);
				offset.setY(offset.y()+4);
			}
#endif			
			...
			if (value && PyTuple_Check(value))
			{
				...
				if (atype)
				{
					...
					} else if (!strcmp(atype, "mtext"))
					{
						...
						if(local_style->m_selection)
						{
							item_right-=eSize(0,6);
							para = new eTextPara(eRect(offset + eSize(xoffs-7, 5), item_right));
						}
						else
							para = new eTextPara(eRect(offset + eSize(xoffs, 0), item_right));
#else
...
#ifdef BUILD_VUPLUS /* ikseong  */			
									if(local_style->m_selection)
										bbox = eRect(left, offset.y()+3, right-left, m_itemsize.height()-8);
									else
#endif				
