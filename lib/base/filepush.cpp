...
#define PVR_ABORT 2
	while(1){
		int ret;
		::ioctl(m_fd_dest, PVR_ABORT);  
		ret = sendSignal(SIGUSR1);
		if(!ret || (ret == -1))
		break;
		if(ret == ESRCH )
			eDebug("No thread could be found corresponding to that specified by the given thread ID");
		else if(ret == EINVAL )
			eDebug("The value of the sig argument is an invalid or unsupported signal number");
		else if (ret == EINTR)
			eDebug("shall not return an error code of [EINTR] what is wrong??");

		eDebug("send SIGUSR1 to thread context");

		usleep(50000); // wait msek
	}
#endif
	kill(0);
}
